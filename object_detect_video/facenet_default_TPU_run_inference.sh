...
VIDEO_DIR=/mnt/FACENET_VIDEOS_RSS_2019/
...
MODEL_NAME=default_facenet

for video_num in trial_2;
do
    ...
    #python3 csv_run_inference.py --video_num $video_num --base-video-dir ${BASE_VIDEO_DIR} --model ${MODEL} --confidence ${CONFIDENCE} --maxobjects ${MAX_OBJECTS} --model_name ${MODEL_NAME} --csv_annotations ${OUTPUT_CSV_DIR} --output-video-dir ${OUTPUT_VIDEO_DIR} --ct $CPU_TPU

    # MUCH SLOWER, create a CSV and video too
    python3 csv_run_inference.py --video_num $video_num --base-video-dir ${BASE_VIDEO_DIR} --model ${MODEL} --confidence ${CONFIDENCE} --maxobjects ${MAX_OBJECTS} --model_name ${MODEL_NAME} --csv_annotations ${OUTPUT_CSV_DIR} --output-video-dir ${OUTPUT_VIDEO_DIR} --ct $CPU_TPU --out-video-create-mode 'True'
